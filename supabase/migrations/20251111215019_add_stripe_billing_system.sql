/*
  # Stripe-Backed Billing System with Dynamic Inline Pricing

  ## Overview
  Comprehensive billing system supporting three plans: Pay Per Use (PPU), Unlimited, and Complimentary.
  Uses Stripe for payment processing with dynamic inline pricing for maximum flexibility.
  ...
  - Deprecate old columns (keep for migration safety): payment_model, wallet_balance, monthly_fee
  ...
  - `rate_at_time_cents` (integer, rate that was active when call occurred)
  ...
  - `id` (uuid, primary key)
  - `user_id` (uuid, references users)
  - `type` (text: 'top_up', 'deduction', 'admin_credit', 'admin_debit', 'refund')
  ...
  - `subtotal_cents` (integer, amount before wallet)
  - `wallet_applied_cents` (integer, wallet credits used)
  - `total_charged_cents` (integer, amount charged to card)
  - `status` (text: 'draft', 'finalized', 'paid', 'failed', 'cancelled')
  ...
  - Usage logs are read-only for users, admins can modify
...
-- Add comment to deprecated columns
...
-- Function to reset monthly tracking on billing cycle

...
export function AdminConfigPage() {
  const [activeTab, setActiveTab] = useState<'connections' | 'sync' | 'audit'>('connections');
  const [syncStatus, setSyncStatus] = useState<SyncStatus | null>(null);
  const [connectionsStatus, setConnectionsStatus] = useState<any[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      if (activeTab === 'connections') {
        const status = await adminService.getConnectionsStatus();
          ...
          {activeTab === 'connections' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-semibold text-gray-900">User Connections</h2>
                <button
                  onClick={() => loadData()}
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {connectionsStatus.length === 0 ? (
                      ...
                      connectionsStatus.map((user: any) => (
                        <tr key={user.user_id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div>
                              ...
                              {user.business_name && (
                                <div className="text-xs text-gray-400 mt-0.5">{user.business_name}</div>
                              )}
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-2">
                              <div className={`h-2.5 w-2.5 rounded-full ${user.has_connection ? 'bg-green-500' : 'bg-red-500'}`} />
                              <span className="text-sm text-gray-700">
                                {user.has_connection ? 'Connected' : 'Not Connected'}
                              </span>
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-2">
                              <div className={`h-2.5 w-2.5 rounded-full ${user.token_status === 'valid' ? 'bg-green-500' : 'bg-red-500'}`} />
                              <span className="text-sm text-gray-700">
                                {user.token_status === 'valid' ? 'Valid' : user.token_status === 'expired' ? 'Expired' : 'Missing'}
                              </span>
                            </div>
                            {user.token_expires_at && (
                              <div className="text-xs text-gray-500 mt-1">
                                Expires: {format(new Date(user.token_expires_at), 'MMM d, yyyy')}
                              </div>
                            )}
                          </td>
                        </tr>
